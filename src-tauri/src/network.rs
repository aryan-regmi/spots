...
impl Network {
    ...
    pub async fn init_endpoint(&mut self, app_handle: &AppHandle, user_id: i64) -> Result<()> {
        // Get secret key from database
        let db = app_handle.state::<Database>();
        let secret_key = {
            let encrypted_key = db.get_secret_key(user_id).await?;
    ...
    pub async fn init_topic(&mut self, app_handle: &AppHandle, user_id: i64) -> Result<()> {
        let db = app_handle.state::<Database>();
        ...
        let topic = self
            .gossip
            .as_ref()
            .ok_or(anyhow::anyhow!("Invalid `Gossip`"))?
            .subscribe(topic_id, peers.clone())
            .await?;
        let (sender, receiver) = topic.split();

        // Add topic data to the database
        db.set_topic_id(user_id, topic_id.to_string()).await?;
        db.set_peers(user_id, Peers { nodes: peers }).await?;
    ...
    pub async fn load_topic(&mut self, app_handle: &AppHandle, user_id: i64) -> Result<()> {
        let db = app_handle.state::<Database>();

        // Load topic from database
        let topic_id = {
            if let Some(id) = db.get_topic_id(user_id).await? {
                TopicId::from_str(&id)?
            } else {
        ...
        let peers = db.get_peers(user_id).await?.nodes;
...
impl EncryptedValue {
    ...
    pub fn decrypt_to_bytes(self) -> Result<Vec<u8>> {
        ...
        let nonce_bytes: [u8; 12] = self
            .nonce
            .try_into()
            .map_err(|_| anyhow::anyhow!("Nonce must be exactly 12 bytes"))?;
        ...
        // Decrypt
        Ok(cipher.decrypt(nonce, self.value.as_slice())?)
    }
}
