...
export async function passwordIsCorrect(db: Database, username: string, password: string) {
  const dbPassword = await db.select<{ password: string }[]>(
    `SELECT password FROM users WHERE username = $1 LIMIT 1`,
    [username]
  );
  console.log(`${dbPassword[0].password}: ${password} ==> ${dbPassword[0].password === password}`)
